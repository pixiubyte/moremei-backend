...
func (e *executor) Start() {
	go func() {
		if r := recover(); r != nil {
			e.log.Error("xxljob注册执行器出错：%s，%s", r, debug.Stack())
		}
		e.registry()
	}()
	go func() {
		if r := recover(); r != nil {
			e.log.Error("xxljob启动web服务器出错：%s，%s", r, debug.Stack())
		}
		err := e.runWebServe()
		if err != nil {
			e.log.Error("xxljob启动web服务器出错：%s", err)
		}
	}()
}
...
func (e *executor) runWebServe() (err error) {
	// 创建路由器
