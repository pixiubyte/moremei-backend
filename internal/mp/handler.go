...
var (
	...
	EventScanHandler                  func(*EventSubscribe) ReplyMsg
...
var RecvDefaultHandler = func(msg *Message) (reply ReplyMsg) {
	logx.Debugf("unregistered receive message handler %s, use RecvDefaultHandler", msg.MsgType)
	return nil
}

// EventDefaultHandler 如果没有注册某类事件处理器，那么收到这类事件时，使用这个默认处理器
var EventDefaultHandler = func(msg *Message) (reply ReplyMsg) {
	logx.Debugf("unregistered receive event handler %s, use EventDefaultHandler", msg.Event)
	return nil
}

// HandleMessage 处理各类消息
func HandleMessage(msg *Message) (ret ReplyMsg) {
	logx.Debugf("process `%s` message", msg.MsgType)
...
func HandleEvent(msg *Message) (reply ReplyMsg) {
	logx.Infof("process `%s` event", msg.MsgType)
	...
	case EventTypeScan:
		if EventScanHandler != nil {
			return EventScanHandler(NewEventSubscribe(msg))
		}
