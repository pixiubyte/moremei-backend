...
const (
	...
	// convertToOpenidURL 外部联系人openid转换
	convertToOpenidURL = "https://qyapi.weixin.qq.com/cgi-bin/externalcontact/convert_to_openid"
)
...
func (r *Client) ConvertToOpenid(request ConvertToOpenidRequest) (string, error) {
	accessToken, err := r.GetAccessToken()
	if err != nil {
		return "", err
	}
	...
	response, err = util.HTTPPost(fmt.Sprintf("%s?access_token=%v", convertToOpenidURL, accessToken), string(jsonData))
	if err != nil {
		return "", err
	}
	var result ConvertToOpenidResponse
